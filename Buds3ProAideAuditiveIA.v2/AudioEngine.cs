...
namespace Buds3ProAideAuditiveIA.v2
{
    ...
    public sealed class AudioEngine : Java.Lang.Object
    {
        ...
        {
            int underruns = -1;
            try { underruns = _trk?.UnderrunCount ?? -1; } catch { }
            return new AudioHealthMetrics
            {
                BufferUnderrunsCount = underruns,
                LatencyMs = EstimatedLatencyMs,
                CpuUsagePercent = -1 // TODO: hook if needed
            };
        }

        public enum AudioPreset { Default, Restaurant, Office, Outdoor }
        ...
        {
            _transport = t;
            _log.Log($"[Route] Transport set to {t}"); try { LogUtilities.Log(_ctx, "ROUTE", $"Transport set to {t}"); } catch { }
        }
        ...
        // Gain
        public void SetGainDb(int db) { _gainDb = Clamp(db, 0, 36); _gainMultiplier = Math.Pow(10.0, _gainDb / 20.0); }
...
#pragma warning restore CS0618
                ...
                _log.Log($"[Audio] RUN ▶ fs={_fs} frame={_n} samples recBuf={_recBufBytes} trkBuf={_trkBufBytes} transport={_transport}"); try { LogUtilities.LogLatency(_ctx, TransportLatencyMs, AlgoLatencyMs); LogUtilities.LogRoute(_ctx, BluetoothRouting_Utilities.GetActiveRouteInfo(_ctx)); } catch { }
                return true;
            }
            catch (Exception ex)
            {
        ...
        private void PushMeters(short[] buf, int len, float grDb)
        {
            ...
            // RMS/peak robust aux overflows 16-bit
            double sum = 0;
            int pk = 0;
            for (int i = 0; i < len; i++)
            {
            ...
            // pk peut valoir 32768 théorique ; borne à 32767 pour la référence dB
